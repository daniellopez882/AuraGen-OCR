...
from auragen.debug.draw import draw_bboxes_on_image
...
@click.command(help="Benchmark auragen layout model.")
@click.option(
    "--results_dir",
    type=str,
...
def main(results_dir: str, max_rows: int, debug: bool):
    ...
    label_alignment = {  # First is publaynet, second is auragen
    ...
    for idx, pred in enumerate(layout_predictions):
        ...
        for label_name in label_alignment:
            correct_cats, auragen_cats = label_alignment[label_name]
            correct_bboxes = [
                b
            ...
            pred_bboxes = [b.bbox for b in pred.bboxes if b.label in auragen_cats]
    ...
    print(
        f"Took {auragen_time / len(images):.5f} seconds per image, and {auragen_time:.5f} seconds total."
    )
    print(
