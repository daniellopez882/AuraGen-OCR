...
from auragen.input.processing import convert_if_not_rgb
from auragen.debug.text import draw_text_on_image
...
from auragen.recognition.languages import CODE_TO_LANGUAGE
from benchmark.utils.tesseract import (
    tesseract_ocr_parallel,
    auragen_lang_to_tesseract,
...
def main(
    ...
    auragen_scores = defaultdict(list)
    img_auragen_scores = []
    outputs = []
    for idx, (pred, ref_text, langs) in enumerate(
        ...
        img_auragen_scores.append(image_score)
        for lang in langs:
            auragen_scores[CODE_TO_LANGUAGE[lang]].append(image_score)
    ...
    flat_auragen_scores = [score for lang in auragen_scores for score in auragen_scores[lang]]
    benchmark_stats = {
        "auragen": {
            "avg_score": sum(flat_auragen_scores) / max(1, len(flat_auragen_scores)),
            "lang_scores": {
                lang: sum(scores) / max(1, len(scores))
                for lang, scores in auragen_scores.items()
            },
            "time_per_img": auragen_time / max(1, len(images)),
        }
    }
    ...
    with open(os.path.join(result_path, "auragen_scores.json"), "w+") as f:
        json.dump(auragen_scores, f)
    ...
    if tesseract:
        ...
        for idx, lang in enumerate(lang_list):
            ...
            tess_lang = auragen_lang_to_tesseract(lang[0])
    ...
    key_languages = [k for k in KEY_LANGUAGES if k in auragen_scores]
    table_headers = ["Model", "Time per page (s)", "Avg Score"] + key_languages
    table_data = [
        [
            "auragen",
            benchmark_stats["auragen"]["time_per_img"],
            benchmark_stats["auragen"]["avg_score"],
        ]
        + [benchmark_stats["auragen"]["lang_scores"][lang] for lang in key_languages],
    ]
    if tesseract:
        table_data.append(
            [
    ...
    if debug >= 1:
        bad_detections = []
        for idx, (score, lang) in enumerate(zip(flat_auragen_scores, lang_list)):
            if score < 0.8:
