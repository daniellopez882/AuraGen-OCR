...
from auragen.input.processing import open_pdf, get_page_images, convert_if_not_rgb
from auragen.debug.draw import draw_polys_on_image
from auragen.common.util import rescale_bbox
...
def main(pdf_path: str, results_dir: str, max_rows: int, debug: bool, tesseract: bool):
    ...
    for idx, (tb, sb, cb) in enumerate(zip(tess_predictions, predictions, correct_boxes)):
        auragen_boxes = [s.bbox for s in sb.bboxes]
        auragen_polys = [s.polygon for s in sb.bboxes]

        auragen_metrics = precision_recall(auragen_boxes, cb)
        if tb is not None:
        ...
        page_metrics[idx] = {
            "auragen": auragen_metrics,
            "tesseract": tess_metrics
        }

        if debug:
            bbox_image = draw_polys_on_image(auragen_polys, copy.deepcopy(images[idx]))
            bbox_image.save(os.path.join(result_path, f"{idx}_bbox.png"))

    mean_metrics = {}
    metric_types = sorted(page_metrics[0]["auragen"].keys())
    models = ["auragen"]
    if tesseract:
    ...
    table_data = [
        ["auragen", auragen_time, auragen_time / len(images)] + [mean_metrics["auragen"][m] for m in metric_types],
    ]
    if tesseract:
        table_data.append(
