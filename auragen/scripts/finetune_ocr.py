...
from auragen.common.auragen import auragenModel
from auragen.common.auragen.processor import auragenOCRProcessor
from auragen.foundation import FoundationPredictor
from auragen.common.auragen.processor.schema import ImageInput, TextInput
from auragen.common.auragen.schema import TaskNames
from auragen.common.util import get_top_scripts, SCRIPT_TOKEN_MAPPING
...
class auragenOCRDataset(torch.utils.data.Dataset):
    def __init__(self, processor: auragenOCRProcessor, data_args: auragenOCRDataArguments):
        super().__init__()
...
def load_model_and_processor(checkpoint_path: Optional[str] = None) -> Tuple[auragenModel, auragenOCRProcessor]:
...
@dataclass
class auragenOCRModelArguments:
    pretrained_checkpoint_path: Optional[str] = field(default=None)

@dataclass
class auragenOCRDataArguments:
...
@dataclass
class auragenOCRTrainingArguments(TrainingArguments):
    remove_unused_columns: bool = field(default=False)
    
def main():
    parser = HfArgumentParser((auragenOCRModelArguments, auragenOCRDataArguments, auragenOCRTrainingArguments))
    model_args, data_args, training_args = parser.parse_args_into_dataclasses()

    model, processor = load_model_and_processor(model_args.pretrained_checkpoint_path)
    dataset = auragenOCRDataset(processor, data_args)
    collator = auragenOCRDataCollator(processor, data_args)
