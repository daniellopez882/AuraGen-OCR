...
from auragen.common.s3 import S3DownloaderMixin
from auragen.common.auragen.config import auragenModelConfig
from auragen.common.auragen.decoder import auragenDecoderModel
from auragen.common.auragen.embedder import SimpleTokenEmbedder
from auragen.common.auragen.encoder import auragenEncoderModel
from auragen.common.util import pad_to_batch_size, pad_to_batch_size_repeat
from auragen.common.xla import get_nearest_pad
...
@dataclass
class auragenModelOutput(CausalLMOutputWithPast):
...
class auragenModel(S3DownloaderMixin, auragenPreTrainedModel):
    config_class = auragenModelConfig
    ...
    def __init__(
        self,
        config: auragenModelConfig,
        embedder: SimpleTokenEmbedder = None,
        vision_encoder: auragenEncoderModel = None,
        decoder: auragenDecoderModel = None,
        **kwargs,
    ):
        super().__init__(config, **kwargs)

        if vision_encoder is None:
            vision_encoder = auragenEncoderModel(config.vision_encoder)

        if decoder is None:
            decoder = auragenDecoderModel(config.decoder)
    ...
    def forward(
        ...
        **kwargs: KwargsForCausalLM,
        ...
        if self.decoder.config._attn_implementation == "flash_attention_2" and prefill:
            # Needed for CPU -> GPU
            from auragen.common.auragen.flash_attn_utils import _get_unpad_data
...
class auragenXLAModel(auragenModel):
    def get_image_embeddings(
        self,
        pixel_values: torch.Tensor,
        grid_thw: torch.Tensor,
