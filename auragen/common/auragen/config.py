...
class auragenModelConfig(S3DownloaderMixin, PretrainedConfig):
    model_type = "auragen-multimodal-foundation"
    is_composition = True
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(vision_encoder, dict):
            vision_encoder = auragenEncoderConfig(**vision_encoder)
        elif vision_encoder is None:
            vision_encoder = auragenEncoderConfig()
        self.vision_encoder = vision_encoder

        if isinstance(decoder, dict):
            decoder = auragenDecoderConfig(**decoder)
        elif decoder is None:
            decoder = auragenDecoderConfig()
