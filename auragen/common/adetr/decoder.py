...
from auragen.common.pretrained import auragenPreTrainedModel
from auragen.common.xla import mark_step
...
class auragenADETRDecoderRMSNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        # Llama does x.to(float16) * w whilst auragenADETRDecoder is (x * w).to(float16)
...
ALL_LAYERNORM_LAYERS.append(auragenADETRDecoderRMSNorm)


class auragenADETRDecoderRotaryEmbedding(nn.Module):
    ...
    @torch.no_grad()
    # Copied from transformers.models.gemma.modeling_gemma.GemmaRotaryEmbedding.forward with Gemma->auragenADETRDecoder
...
class auragenADETRDecoderSdpaCrossAttention(nn.Module):
    ...
    def __init__(self, config: PretrainedConfig):
        ...
        self.rotary_emb = auragenADETRDecoderRotaryEmbedding(
            self.head_dim,
...
class auragenADETRDecoderSdpaAttention(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""
...
class auragenADETRDecoderMlp(nn.Module):
...
class auragenADETRDecoderLayer(nn.Module):
    def __init__(self, config, layer_idx, static_cache=False, max_boxes=None):
        super().__init__()
        self.cross_pre_norm = auragenADETRDecoderRMSNorm(
            config.hidden_size, eps=config.rms_norm_eps
        )
        self.temporal_pre_norm = auragenADETRDecoderRMSNorm(
            config.hidden_size, eps=config.rms_norm_eps
        )

        self.temporal_block = None
        if layer_idx in config.self_attn_layers:
            self.temporal_block = auragenADETRDecoderSdpaAttention(
                config, static_cache=static_cache, max_boxes=max_boxes
            )

        self.cross_attn_block = None
        if layer_idx in config.cross_attn_layers:
            self.cross_attn_block = auragenADETRDecoderSdpaCrossAttention(config)

        self.window_attn = layer_idx not in config.global_attn_layers
        self.channel_pre_norm = auragenADETRDecoderRMSNorm(
            config.hidden_size, eps=config.rms_norm_eps
        )
        self.mlp_block = auragenADETRDecoderMlp(config)
...
class auragenADETRDecoderPreTrainedModel(auragenPreTrainedModel):
    config_class = PretrainedConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["auragenADETRDecoderLayer"]
    ...
    def _init_weights(self, module):
        if isinstance(module, auragenADETRDecoderSdpaAttention):
...
class auragenADETRDecoderModel(auragenADETRDecoderPreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`auragenADETRDecoderDecoderLayer`]
